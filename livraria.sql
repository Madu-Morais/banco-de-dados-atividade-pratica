...
select count(idLivro) as 'Quantidade Livro' from livro;

select nome from cliente order by nome asc;

select e.nome as Nome_Editora, l.titulo as Título_Livro 
from editora e
join livro l on e.idEditora = l.idEditora
order by e.nome desc;

select e.nome as Nome_Editora, avg(l.preco) as Media_Preco_Livros
from editora e
join livro l on e.IdEditora = l.idEditora
group by e.nome;

select c.nome as Nome_cliente, count(i.IdLivro)
from cliente c
